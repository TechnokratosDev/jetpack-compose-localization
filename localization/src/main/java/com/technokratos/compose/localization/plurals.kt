...
fun Plurals(
    ...
    for ((locale, value) in localeToPlural().entries) {
        if (PluralRulesHost.checkLocale(locale)) {
            Log.w("jcl10n", "There is no plural rule for $locale currently. Plural's 'other = ${value.other}' field can be used only!")
        }
        val localization =
...
fun Localization.getPlural(name: String, quantity: Double): CharSequence? {
    ...
    return plurals[name]?.let { PluralRulesHost.resolveString(it, locale, absQuantity, integerPart, fractionPart, fractionPartDigitCount) }
}

private object PluralRulesHost {
    ...
    fun checkLocale(locale: Locale): Boolean {
        return localeToPluralRule.containsKey(locale)
    }

    fun resolveString(plural: Plural, locale: Locale, n: Double, i: Long, f: Long, v: Int): CharSequence {
